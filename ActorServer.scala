...
object ActorServer {
  def main(args: Array[String]) = {
    ...
    val system = ActorSystem("Response")
    
    while(true) {
      val actor = system.actorOf(Props[ConResponse])
...
class ConResponse extends Actor {    
  case class ResponseHeader(
    status: String, 
    date: String = "Date: " + new java.util.Date,
    server: String = "Server: SSWS/0.01 (Simple Scala Web Server - Actor)",
  ...
  def receive = {
    case clientSocket: Socket =>
      ...
      val actorId = "Actor: " + self.toString + '\n'
